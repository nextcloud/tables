...
test.describe('Filtering in a view by selection columns', () => {
	...
	test('Filter view removes rows that no longer match', async ({ userPage: { page } }) => {
		await createFilteredView(page, 'Filter for check enabled', [[
			{ column: 'check', operator: 'Is equal', value: 'Checked' },
		]])

		await page.locator('[data-cy="createRowBtn"]').click()
		await fillInValueTextLine(page, 'title', 'checked row')
		await fillInValueSelectionCheck(page, 'check')
		const createCheckedPresenceReq = page.waitForResponse((response) =>
			/apps\/tables\/view\/\d+\/row\/\d+\/present/.test(response.url())
			&& response.request().method() === 'GET',
		)
		await page.locator('[data-cy="createRowSaveButton"]').click()
		await expect((await createCheckedPresenceReq).json()).resolves.toMatchObject({ present: true })
		await expectVisibleRows(page, ['checked row'])

		await page.locator('[data-cy="createRowBtn"]').click()
		await fillInValueTextLine(page, 'title', 'unchecked row')
		const createUncheckedPresenceReq = page.waitForResponse((response) =>
			/apps\/tables\/view\/\d+\/row\/\d+\/present/.test(response.url())
			&& response.request().method() === 'GET',
		)
		await page.locator('[data-cy="createRowSaveButton"]').click()
		await expect((await createUncheckedPresenceReq).json()).resolves.toMatchObject({ present: false })
		await expectMissingRows(page, ['unchecked row'])

		await page.locator('[data-cy="customTableRow"]').filter({ hasText: 'checked row' }).locator('[data-cy="editRowBtn"]').click()
		await page.locator('[data-cy="editRowModal"] .checkbox-radio-switch').click()
		const editPresenceReq = page.waitForResponse((response) =>
			/apps\/tables\/view\/\d+\/row\/\d+\/present/.test(response.url())
			&& response.request().method() === 'GET',
		)
		await page.locator('[data-cy="editRowSaveButton"]').click()
		await expect((await editPresenceReq).json()).resolves.toMatchObject({ present: false })
		await expectMissingRows(page, ['checked row'])

		const inlinePresenceReq = page.waitForResponse((response) =>
			/apps\/tables\/view\/\d+\/row\/\d+\/present/.test(response.url())
			&& response.request().method() === 'GET',
		)
		await page
			.locator('[data-cy="customTableRow"]')
			.filter({ hasText: 'first row' })
			.locator('.inline-editing-container input')
			.click({ force: true })
		await expect((await inlinePresenceReq).json()).resolves.toMatchObject({ present: false })
		await expectMissingRows(page, ['first row'])
	})
})
