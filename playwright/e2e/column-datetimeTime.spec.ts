...
test.describe('Test column ' + columnTitle, () => {
	...
	test('Insert and test rows - default now', async ({ userPage: { page } }) => {
		...
		await loadTable(page, tableTitle)
		await createDatetimeTimeColumn(page, columnTitle, true, true)

		await page.locator('button').filter({ hasText: 'Create row' }).click()
		await expect(page.locator('.modal__content input').first()).toHaveValue(/07:21/)

		await page.locator('button').filter({ hasText: 'Save' }).click()

		await expect(page.locator('.custom-table table tr td div').filter({ hasText: '7:21' }).first()).toBeVisible()
	})
})
