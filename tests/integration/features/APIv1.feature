...
@api1 @contexts @contexts-sharing
  ...
  @api1 @views @technical-name
  Scenario: Create view with duplicate technical name should fail
    Given table "View duplicate test" with emoji "🔁" exists for user "participant1" as "duplicate-table"
    When user "participant1" create view "First View" with emoji "1️⃣" and technical name "duplicate_view" for "duplicate-table" as "first-view"
    When user "participant1" attempts to create view "Second View" with emoji "2️⃣" and technical name "duplicate_view" for "duplicate-table" as "second-view"
    Then the reported status is "400"
    When user "participant1" deletes view "first-view"
