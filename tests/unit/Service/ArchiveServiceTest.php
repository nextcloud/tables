...
class ArchiveServiceTest extends TestCase {
	...
	public function testArchiveForUserOwnerSetsEntityFlagAndKeepsOverrides(): void {
		$this->mapper->expects($this->never())->method('upsert');
		// existing overrides are sticky: an owner change must never clear them
	...
	public function testUnarchiveForUserOwnerClearsEntityFlagAndKeepsOverrides(): void {
		$this->mapper->expects($this->never())->method('upsert');
		// a recipient who archived it for themselves keeps that override
		$this->mapper->expects($this->never())->method('deleteForUser');
		$this->mapper->expects($this->never())->method('deleteAllForNode');
