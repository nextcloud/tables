...
class ArchiveService {
	...
	public function archiveForUser(string $userId, int $nodeType, int $nodeId, bool $isOwner, bool $entityArchived = false): void {
		if ($isOwner) {
			// Repeating the owner's choice must not clear the overrides of
			// users who deliberately opted out of it in the meantime.
			if ($entityArchived) {
				return;
			}
			$this->atomic(function () use ($nodeType, $nodeId): void {
	...
	public function unarchiveForUser(string $userId, int $nodeType, int $nodeId, bool $isOwner, bool $entityArchived): void {
		if ($isOwner) {
			// See archiveForUser(): repeating the current state is a no-op.
			if (!$entityArchived) {
				return;
			}
			$this->atomic(function () use ($nodeType, $nodeId): void {
