...
class ImportService extends SuperService {
	...
	private function getPreviewData(Worksheet $worksheet): array {
		...
		foreach ($worksheet->getRowIterator(2) as $row) {
			...
			foreach ($cellIterator as $cellIndex => $cell) {
				...
				if (($column && $column->getType() === 'datetime') || $columns[$cellIndex]['type'] === 'datetime') {
					$value = Date::excelToDateTimeObject($value)->format('Y-m-d H:i');
				} elseif (($column && $column->getType() === 'number' && $column->getNumberSuffix() === '%')
					|| ($columns[$cellIndex]['type'] === 'number' && $columns[$cellIndex]['number_suffix'] === '%')) {
					$value = $value * 100;
				} elseif (($column && $column->getType() === 'selection' && $column->getSubtype() === 'check')
					|| ($columns[$cellIndex]['type'] === 'selection' && $columns[$cellIndex]['subtype'] === 'check')) {
	...
	public function import(?int $tableId, ?int $viewId, string $path, bool $createMissingColumns = true, array $columnsConfig = []): array {
		...
		//$columnsConfig = json_decode('[{"titleRaw":"What","title":"What","action":"exist","existColumn":{"id":5,"label":"What"},"type":"text","subtype":"line","typeFull":"text-line","typeLabel":"Text","numberDecimals":0,"numberPrefix":""},{"type":"text","subtype":"line","title":"How to do","description":"","selectedViewIds":[],"mandatory":false,"viewId":null,"tableId":2,"textDefault":"","textMaxLength":null,"titleRaw":"How to do","action":"new","typeFull":"text-line","typeLabel":"Text"},{"titleRaw":"Ease of use","title":"Ease of use","action":"new","existColumn":null,"type":"number","subtype":null,"typeFull":"number","typeLabel":"Number","numberDecimals":0,"numberPrefix":""},{"titleRaw":"Done","title":"Done","action":"new","existColumn":null,"type":"selection","subtype":"check","typeFull":"selection-check","typeLabel":"Selection","numberDecimals":0,"numberPrefix":""}]');
