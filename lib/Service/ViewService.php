...
class ViewService extends SuperService {
	...
	public function create(
		...
		try {
			...
			$this->handleViewPersistDbException($e, 'create view insert error');
		}
		if ($newItem->getTechnicalName() === null || $newItem->getTechnicalName() === '') {
			...
			try {
				...
				$this->handleViewPersistDbException($e, 'create view update error');
			}
		}
	...
	public function update(int $id, ViewUpdateInput $data, ?string $userId = null, bool $skipTableEnhancement = false): View {
		...
		try {
			...
			return $view;
		} catch (BadRequestError|InvalidArgumentException $e) {
			throw $e;
		} catch (\OCP\DB\Exception $e) {
			$this->handleViewPersistDbException($e, 'update view error');
		} catch (Exception $e) {
			$this->logger->error($e->getMessage(), ['exception' => $e]);
			throw new InternalError($e->getMessage());
		}
	...
	/**
	 * The ids of the columns the view may use.
	 *
	 * ColumnService depends on this service, so it is looked up rather than injected.
	 *
	 * @return list<int>
	 */
	private function findManagedColumnIds(View $view, string $userId): array {
	...
	/**
	 * @param list<int> $availableColumnIds
	 */
	protected function assertInputColumnsAreValid(ColumnSettings $columnSettings, array $availableColumnIds): void {
	 ...
	 */
	protected function dropOrphanedCardSources(View $view, array $viewColumnIds): void {
		// Unlike the assertion, this one writes: with no candidates to compare against it
		// leaves the stored sources alone rather than clearing them on a bad lookup.
		...
		$view->setViewSettings(json_encode(new ViewSettings(
	...
	private function assertTechnicalNameValid(string $technicalName): void {
		...
		if (!preg_match('/^[a-z][a-z0-9_]*$/', $technicalName)) {
			throw new BadRequestError('Technical name must start with a lowercase letter and contain only lowercase letters, numbers, and underscores.');
		}
	}
