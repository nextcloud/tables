...
class ShareService extends SuperService {
	...
	private function findElementsSharedWithMe(string $elementType = 'table', ?string $userId = null): array {
		...
		$shares = [];
		try {
			$shares['shared_for_user'] = $this->mapper->findAllSharesFor($elementType, [$userId], $userId);

			$userGroups = $this->userHelper->getGroupsForUser($userId);
			$userGroupIds = array_map(static fn ($group) => $group->getGid(), $userGroups);
			$shares['shared_for_groups'] = $this->mapper->findAllSharesFor($elementType, $userGroupIds, $userId, ShareReceiverType::GROUP);
			...
			$shares['shared_for_circles'] = $this->mapper->findAllSharesFor($elementType, $userCircleIds, $userId, ShareReceiverType::CIRCLE);
		} catch (Throwable $e) {
			throw new InternalError($e->getMessage(), previous:  $e);
		}

		$elementIds = [];
		foreach (array_merge($shares['shared_for_user'], $shares['shared_for_groups'], $shares['shared_for_circles']) as $share) {
			/** @var Share $share */
