...
class RowOCSController extends AOCSController {
	 ...
	 * @psalm-param int<1,500>|null $limit Number of rows to return between 1 and 500, fetches all by default (optional)
	 * @psalm-param ?int<0,max>|null $offset Offset of the tows to be returned (optional)
	...
	public function getRows(string $nodeCollection, int $nodeId, ?int $limit, ?int $offset, ?array $filter, ?array $sort): DataResponse {
		...
		// TODO: add range checks for limit and offset, as optional parameters are only covered through AppFramework in NC 32 cf https://github.com/nextcloud/server/pull/51116
	...
	protected function assertFilterValue(array $filter): void {
		...
		if (!is_numeric($filter['columnId']
			...
			|| !preg_match('/^\d{0,' . $maxDigits .'}$/', $filter['columnId'])
		)) {
	...
	protected function assertSortValue(array $sort): void {
		...
		if (!is_numeric($sort['columnId']
			|| (int)$sort['columnId'] < -5
			|| !preg_match('/^\d{0,' . $maxDigits .'}$/', $sort['columnId'])
		)) {
			throw new InvalidArgumentException('Invalid column id supplied');
		}
