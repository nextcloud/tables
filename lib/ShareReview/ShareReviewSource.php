...
class ShareReviewSource implements IPaginatedShareReviewSource {
	...
	public function countSharesByInitiator(ShareReviewQuery $query, int $limit): array {
		...
		try {
			return $this->shareMapper->countByInitiatorForShareReview($query, $limit, $this->receiverTypes($query), $this->permissionColumns($query));
		} catch (Exception $e) {
			$this->logger->error('Tables ShareReview: failed to count shares by initiator: {message}', ['message' => $e->getMessage()]);
			return [];
		}
	}
