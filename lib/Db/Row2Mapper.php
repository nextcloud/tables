...
class Row2Mapper {
	...
	public function delete(Row2 $row): Row2 {
		...
		try {
			foreach ($this->columnsHelper->columns as $columnType) {
				$this->columnsHelper->getCellMapperFromType($columnType)->deleteAllForRow($rowId);
			}
			$this->rowSleeveMapper->deleteById($rowId);
			$this->db->commit();
		} catch (Throwable $e) {
			$this->db->rollBack();
	...
	private function insertOrUpdateCell(int $rowId, int $columnId, $value): array {
		...
		try {
			if ($cellMapper->hasMultipleValues()) {
				$this->atomic(function () use ($cellMapper, $rowId, $columnId, $value, &$cachedCell): void {
