...
class ActivityManager {
	...
	public function getRowName($rowId, $tableId) {
		try {
			$columns = $this->columnMapper->findAllByTable($tableId);
			$textColumns = array_filter($columns, function ($column) {
				return $column->getType() === 'text';
			});
			$row = $this->rowMapper->find($rowId, $textColumns);
			...
			foreach ($data as $index => $cell) {
				if (isset($cell['value']) && is_string($cell['value']) && $cell['value'] !== '') {
					return strlen($cell['value']) <= self::ROW_NAME_MAX_LENGTH
						? $cell['value']
						: substr($cell['value'], 0, self::ROW_NAME_MAX_LENGTH) . '...';
				}
			}
		} catch (\Exception $e) {
			Server::get(LoggerInterface::class)->error('Could not find row for activity entry.', [
				'rowId' => $rowId,
				'tableId' => $tableId,
				'exception' => $e->getMessage()
			]);
