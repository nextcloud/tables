	...
	computed: {
		...
		magicFields() {
			if (this.selectedColumn
				...
				&& this.selectedColumn?.type !== ColumnTypes.Relation
			) {
				const fields = []
				...
				return options
			} else if (this.selectedColumn && this.selectedColumn.type === ColumnTypes.Relation) {
				...
				Object.entries(columnRelations.data).forEach(([, item]) => {
					options.push({
