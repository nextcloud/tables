	...
	<NcDialog v-if="showModal"
		...
		<div class="modal__content">
			...
			<div v-if="!loading && !importInitialized && preview === null && !waitForReload">
				...
				<div class="row">
					<div class="fix-col-4 end">
						<NcButton :aria-label="t('tables', 'Preview')" variant="primary" @click="actionPreview">
							{{ t('tables', 'Preview') }}
						</NcButton>
					</div>
				</div>
			...
			<div v-if="!loading && preview !== null && !importInitialized && !waitForReload">
				<ImportTableSchemePreview :preview-data="preview" :table="table" @update="onUpdateScheme" />

				<div class="row">
					<div class="fix-col-4 space-T justify-between">
						<NcButton :aria-label="t('tables', 'Back')" @click="actionBack">
							{{ t('tables', 'Back') }}
						</NcButton>
						<NcButton :aria-label="t('tables', 'Import')" variant="primary" @click="actionImport">
							{{ t('tables', 'Import') }}
						</NcButton>
					</div>
				</div>
			...
			<div v-if="!loading && importInitialized && result !== null && !waitForReload">
				...
				<div class="row">
					<div class="fix-col-4 space-T end">
						<NcButton :aria-label="t('tables', 'Done')" variant="primary" @click="actionCloseAndReload">
							{{ t('tables', 'Done') }}
						</NcButton>
					</div>
				</div>
	...
	computed: {
		...
		title() {
			...
			if (!this.loading && this.preview !== null && !this.importInitialized && !this.waitForReload) {
				title = t('tables', 'Preview table scheme to import')
			}
			...
			try {
				...
				reader.onload = (evt) => {
					try {
						...
						this.schemeData = this.preview = parsedData
					} catch (parseError) {
