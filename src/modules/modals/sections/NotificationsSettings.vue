			...
			return [
				{
					...
					description: t('tables', 'When row values are created, updated or deleted.'),
				},
				{
					key: 'notify-column',
					label: t('tables', 'Structure changes'),
					description: t('tables', 'When columns are created, updated or deleted.'),
				},
				{
					key: 'notify-assigned',
					label: t('tables', 'Assigned'),
					description: t('tables', 'When you are added in a user or group field.'),
				},
			]
		},
	},
	...
	methods: {
		...
		async updateConfig(configKey, value) {
			...
			try {
				...
				this.config = {
					...this.config,
					[configKey]: response?.data?.ocs?.data === true ? value : previousValue,
				}
			} catch (error) {
				if (this.configScope === targetType && this.elementId === targetId) {
					this.config = {
