						...
						<div class="row space-L space-T space-B">
							...
							<div v-if="column.action === 'new'" class="row no-padding-on-mobile">
								<NcButton @click="editColumn(colIndex)">
									...
									<template #icon>
										<TextLongIcon v-if="['text', 'text-line', 'text-rich'].includes(column.typeFull)" />
										<LinkIcon v-if="column.typeFull === 'text-link'" />
										<CounterIcon v-if="column.typeFull === 'number'" />
										<StarIcon v-if="column.typeFull === 'number-stars'" />
										<ProgressIcon v-if="column.typeFull === 'number-progress'" />
							...
							<div v-if="columnsConfig[colIndex].action === 'exist'" class="row no-padding-on-mobile">
								<NcSelect v-model="columnsConfig[colIndex].existColumn" :options="existColumnOptions" :aria-label-combobox="t('tables', 'Exist column')" />
							</div>
						</div>
					</td>
					<td>
...
export default {
	...
	async mounted() {
		const existColumns = await this.$store.dispatch('getColumnsFromBE', {
			tableId: !this.isView ? this.element.id : null,
			viewId: this.isView ? this.element.id : null,
		})
		this.existColumnOptions = existColumns.map(column => ({
			id: column.id,
			label: column.title,
		}))

		for (let i = 0; i < this.previewData.columns.length; i++) {
			const typeFull = this.previewData.columns[i].type + (this.previewData.columns[i].subtype ? `-${this.previewData.columns[i].subtype}` : '')
			this.columnsConfig.push({
				...
				existColumn: this.previewData.columns[i].id ? this.existColumnOptions.find(col => col.id === this.previewData.columns[i].id) : null,
				type: this.previewData.columns[i].type,
				subtype: this.previewData.columns[i].subtype,
				typeFull,
				typeLabel: this.getTypeLabel(typeFull),
	...
	methods: {
		...
		onSaveColumn(column) {
			...
			this.columnsConfig[this.editingColumnIndex].typeFull = column.type + (column.subtype ? `-${column.subtype}` : '')
			this.columnsConfig[this.editingColumnIndex].typeLabel = this.getTypeLabel(this.columnsConfig[this.editingColumnIndex].typeFull)
			this.editingColumnPreset = null
			this.editingColumnIndex = null
			this.$emit('update:columns', this.columnsConfig)
			this.$forceUpdate()
		},
		getTypeLabel(typeFull) {
			switch (typeFull) {
